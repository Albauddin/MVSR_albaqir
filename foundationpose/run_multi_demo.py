...
if __name__ == '__main__':
    ...
    for frame_str in frame_ids:
        ...
        canvas = color.copy()  # ✅ Start with original RGB
        ...
        for obj_name in object_names:
            ...
            try:
                ...
                # ✅ Draw on cumulative canvas
                canvas = draw_posed_3d_box(reader.K, img=canvas, ob_in_cam=center_pose, bbox=bbox)
                canvas = draw_xyz_axis(canvas, ob_in_cam=center_pose, scale=0.1, K=reader.K, thickness=2)

            except Exception as e:
                logging.error(f"Pose estimation failed for {obj_name} in frame {frame_str}: {e}")

        # ✅ Save only once after all objects are drawn
        vis_path = os.path.join(vis_output_dir, f'{frame_str}_ALL_OBJECTS.png')
        imageio.imwrite(vis_path, canvas)
